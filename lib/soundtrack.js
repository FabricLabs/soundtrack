...
var Soundtrack = function(app) {
  ...
  /**/setInterval(function() {
    self.markAndSweep()
  }, app.config.connection.checkInterval );/**/
}
...
Soundtrack.prototype.ensureQueue = function(callback) {
  ...
  if (app.room.playlist.length === 0) {
    ...
    Play.find( query ).limit(4096).sort('timestamp').exec(function(err, plays) {
      ...
      if (err || !plays || !plays.length) {
        return self.getYoutubeVideo( 'dQw4w9WgXcQ' , function(track) {
          if (track) {
            self.backupTracks.push( track.toObject() );
