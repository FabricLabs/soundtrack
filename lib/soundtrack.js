...
var Soundtrack = function( config ) {
  var self = this;
  self.config = config;
  self.rooms = {};
  self.backupTracks = [];
...
Soundtrack.prototype.start = function( done ) {
  if (!done) var done = new Function();
  var self = this;
  
  self.sub.on('message', function(channel, msg) {
    console.log( msg );
    console.log('ORCH received message on channel', channel , JSON.parse(msg).type );
    ...
    switch (msg.type) {
      case 'skip':
        self.rooms[ msg.data.room ].nextSong();
        break;
      case 'queue':
        console.log('queue will be:', msg.data );
        self.trackFromSource( msg.data.source , msg.data.id , function(err, track) {
          console.log('trackFromSource() callback executing...', err || track._id );
          if (err || !track) return console.error(err);
          
          var queueWasEmpty = false;
          if (!self.rooms[ msg.data.room ].playlist.length) queueWasEmpty = true;
          self.rooms[ msg.data.room ].queueTrack(track, msg.data.user, function() {
            console.log( 'queueTrack() callback executing... ');

            if (queueWasEmpty) {
              self.rooms[ msg.data.room ].broadcast({
                type: 'track',
                data: track,
                //sources: sources,
                seekTo: 0.0
  ...
  /* setInterval(function() {
    self.markAndSweep();
  }, self.config.connection.checkInterval );*/

  Room.find().exec(function(err, rooms) {
    async.map( rooms , function(room, complete) {
      self.sub.get( self.config.database.name + ':rooms:' + room.slug + ':playlist', function(err, playlist) {
        ...
        self.rooms[ room.slug ].playlist = playlist;
        self.rooms[ room.slug ].listeners = {};
        
        self.rooms[ room.slug ].bind( self );
        
        self.sub.subscribe('soundtrack:rooms:' + room.slug );
        self.sub.subscribe('soundtrack:rooms:' + room.slug + ':*' );
        
        function errorHandler(err) {
          if (err) {
            return self.rooms[ room.slug ].retryTimer = setTimeout(function() {
              self.rooms[ room.slug ].startMusic( errorHandler );
            }, 5000 );
          }
...
Soundtrack.prototype.broadcast = function(msg) {
  ...
  self.pub.publish('soundtrack:messages', json );
...
Soundtrack.prototype.trackFromSource = function(source, id, sourceCallback) {
  ...
  switch (source) {
    ...
    break;
    case 'soundcloud':
      rest.get('https://api.soundcloud.com/tracks/'+parseInt(id)+'.json?client_id='+ self.config.soundcloud.id).on('complete', function(data, response) {
